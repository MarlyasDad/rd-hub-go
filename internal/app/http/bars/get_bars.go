...
type (
	getSubscriberBarsCommand interface {
		GetSubscriberBars(subscriberID alor.SubscriberID, heikenAshi bool) ([]*alor.Bar, error)
	}
...
func (h *GetSubscriberBarsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("route %s with error: %s", h.name, err)

		if errors.Is(err, domain.ErrSubscriberNotFound) {
			responses.GetErrorResponse(w, h.name, err, http.StatusNotFound)
			return
		}
	...
	responses.GetSuccessResponse(w, portfoliosJson)
}
